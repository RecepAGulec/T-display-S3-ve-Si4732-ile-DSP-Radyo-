/*
  ...
  3) Siyah ekranda beyaz yazılar çok gözalıcı göründüğünden, camgöbeği (cyan) renkle değiştirildi. 
     RDS yazıları gri yapıldı. Ana frekans gösterge rengi VFD gibiymişçesine
     açık yeşil renge aarlandı.
  ...
  ** ARDUINO IDE İLE DERLEME İÇİN GEREKLİ AYARLAR **
  --------------------------------------------
...
#define PIN_LCD_BL    38           // Arkaışık
#define RESET_PIN     16           // GPIO16
#define AUDIO_MUTE    17           // GPIO17

// Enconder PINs - Kendi bağlantınıza göre değiştirebilirsiniz.
...
char sAgc[15];
const char *menu[] = {"SES", "ADIM", "KiP", "BFO", "BW", "AGC/Att", "SoftMute", "YUK. ARA", "AS. ARA", "ISIK", "SESSiZ"}; //2. menü ögeleri
...
int idxAmStep = 3;
int tabFmStep[] = {5, 10, 20, 100}; // 1 MHz stepsize added for VHF - FM'de 1MHz'lik adım eklendi
...
// 1. menünün ögeleri
Band band[] = {
  {"VHF", FM_BAND_TYPE, 8750, 10800, 10280, 1, 0},
  {"LW ", MW_BAND_TYPE, 149, 531, 356, 0, 5},
  {"MW ", MW_BAND_TYPE, 531, 1701, 783, 2, 5},
  {"MF ", MW_BAND_TYPE, 1700, 3500, 2500, 1, 5},
  {"80H", MW_BAND_TYPE, 3500, 3800, 3500, 0, 4},
  {"75M", SW_BAND_TYPE, 3900, 4000, 4885, 1, 5},
  {"60M", SW_BAND_TYPE, 4750, 5060, 4885, 1, 5},
  {"49M", SW_BAND_TYPE, 5900, 6200, 6000, 1, 5},
  {"40H", SW_BAND_TYPE, 7000, 7300, 7100, 0, 4},
  {"41M", SW_BAND_TYPE, 7200, 7450, 7300,  1,  5},
  {"31M", SW_BAND_TYPE, 9400, 9900, 9500, 1, 5},
  {"30H", SW_BAND_TYPE, 10095, 10150, 10100, 0, 4},
  {"25M", SW_BAND_TYPE, 11600, 12100, 11700, 1, 5},
  {"22M", SW_BAND_TYPE, 13550, 13900, 13600, 1, 5},
  {"20H", SW_BAND_TYPE, 14000, 14360, 14200, 0, 4},
  {"19M", SW_BAND_TYPE, 15090, 15850, 15100, 1, 5},
  {"16M", SW_BAND_TYPE, 17450, 17950, 17500, 1, 5},
  {"17M", SW_BAND_TYPE, 18000, 18300, 18100, 1, 5},
  {"15M", SW_BAND_TYPE, 18890, 19050, 19000, 1, 5},
  {"15H", SW_BAND_TYPE, 21000, 21451, 21100, 0, 4},
  {"13M", SW_BAND_TYPE, 21450, 21850, 21500, 1, 5},
  {"12H", SW_BAND_TYPE, 24880, 24940, 21100, 0, 4},
  {"11M", SW_BAND_TYPE, 25670, 26100, 25700, 1, 5},
  {"CB ", SW_BAND_TYPE, 26000, 28000, 27500, 1, 5},
  {"10M", SW_BAND_TYPE, 28000, 30000, 28400, 0, 4},
  {"ALL", SW_BAND_TYPE, 150, 30000, 15000, 0, 1} // All band. LW, MW and SW (from 150kHz to 30MHz)
};
...
void setup()
{
  ...
  Wire.begin(ESP32_I2C_SDA, ESP32_I2C_SCL);       // I2C başlatılıyor
  tft.begin();                                    // LCD ekran başlatılıyor
  tft.setRotation(3);                             // Ekranın yönü belirleniyor
  ...
  EEPROM.begin(EEPROM_SIZE);                  // If you want to reset the eeprom, keep the ECODER_SW button pressed during startup
  ...
  if ( si4735Addr == 0 ) {
    tft.setTextSize(2);
    tft.setTextColor(TFT_RED, TFT_BLACK);
    tft.println("Si4732/35 bulunamadi");
    while (1);
  }
...
// SSB yükleme
void loadSSB() {
  rx.setI2CFastModeCustom(400000); // You can try rx.setI2CFastModeCustom(700000); or greater value XXXXXXXXXXXXXXXXXXXXXX
...
void doBandwidth(int8_t v)
{
  if (currentMode == LSB || currentMode == USB)
  {
    bwIdxSSB = (v == 1) ? bwIdxSSB - 1 : bwIdxSSB + 1; //// Orj. bwIdxSSB = (v == 1) ? bwIdxSSB + 1 : bwIdxSSB - 1;
  ...
  {
    bwIdxAM = (v == 1) ? bwIdxAM - 1 : bwIdxAM + 1; // Orj.  bwIdxAM = (v == 1) ? bwIdxAM + 1
    if (bwIdxAM > maxAmBw)
      bwIdxAM = 0;
    else if (bwIdxAM < 0)
      bwIdxAM = maxAmBw;
    ...
    bwIdxFM = (v == 1) ? bwIdxFM - 1 : bwIdxFM + 1; // Orj. bwIdxFM = (v == 1) ? bwIdxFM + 1 : bwIdxFM - 1;
    if (bwIdxFM > maxFmBw)
      bwIdxFM = 0;
    else if (bwIdxFM < 0)
      bwIdxFM = maxFmBw;
...
{
  if ( currentMode == FM ) {
    idxFmStep = (v == 1) ? idxFmStep - 1 : idxFmStep + 1; // DİKKAT Orj. idxFmStep = (v == 1) ? idxFmStep + 1 : idxFmStep - 1;
    ...
    idxAmStep = (v == 1) ? idxAmStep - 1 : idxAmStep + 1; // DİKKAT Orj.  idxAmStep = (v == 1) ? idxAmStep + 1 : idxAmStep - 1;
  ...
  if (currentMode != FM)
  {
    if (v == 1)  { // clockwise
      if (currentMode == AM)
      {
        ...
        spr.fillSmoothRoundRect(80, 40, 160, 40, 4, TFT_CYAN); // orj. white
...
//Starts the MENU action process (2. menü ögelerinin seçimi)
...
void drawMenu() {
  if (cmdMenu) {
    spr.fillSmoothRoundRect(1, 1, 76, 110, 4, TFT_MAGENTA);  
    spr.fillSmoothRoundRect(2, 2, 74, 108, 4, TFT_MENU_BACK);
    spr.setTextColor(TFT_CYAN, TFT_MENU_BACK);            
    spr.drawString("Menu", 38, 14, 2);
    spr.setTextFont(0);
    spr.setTextColor(TFT_CYAN, TFT_MENU_BACK);              
    spr.fillRoundRect(6, 24 + (2 * 16), 66, 16, 2, 0x105B);  
     for (int i = -2; i < 3; i++) {
      if (i == 0) spr.setTextColor(TFT_CYAN, 0x105B);      
      else spr.setTextColor(TFT_CYAN, TFT_MENU_BACK);      
      spr.drawString(menu[abs((menuIdx + lastMenu + 1 + i) % (lastMenu + 1))], 38, 64 + (i * 16), 2);
    }
  } else {
    spr.setTextColor(TFT_GREEN, TFT_MENU_BACK);           
    spr.fillSmoothRoundRect(1, 1, 76, 110, 4, TFT_GREEN ); 
    spr.fillSmoothRoundRect(2, 2, 74, 108, 4, TFT_MENU_BACK); 
...
void drawSprite()
{
  ...
  spr.setTextColor(0x08FA0, TFT_BLACK); 
  ...
  if (isMenuMode() or cmdBand) drawMenu();
    ...
    if (agcNdx == 0 && agcIdx == 0) {
      ...
      spr.drawString("A", 48, 64 + (1 * 16), 2);
    } else {
      sprintf(sAgc, "%2.2d", agcNdx);
      spr.drawString("ATTt:", 6, 64 + (1 * 16), 2);
    ...
    else spr.drawString("K", 48, 64 + (2 * 16), 2);
    spr.setTextDatum(MC_DATUM);
  }
  ...
  spr.setTextFont(0);
  spr.setTextColor(TFT_CYAN, TFT_BLACK); 
  spr.drawString("SiNYAL:", 266, 54);
  if (muted) {
    spr.setTextColor(TFT_RED, TFT_BLACK);
    spr.drawString("SESSiZ ", 272, 102, 2);
    spr.setTextColor(TFT_CYAN, TFT_BLACK);
  }
  else {
  
     
    spr.drawString("SES:", 257, 102, 2); 
  ...
  if (currentMode == FM) {
    spr.fillSmoothRoundRect(240, 20, 76, 22, 4, TFT_CYAN); 
  ...
  spr.setTextColor(0x949494, TFT_BLACK); 
...
void DrawBatteryLevel(int batteryLevel) {
  ...
  spr.drawRect(XbatPos, YbatPos, Xbatsiz, Ybatsiz, TFT_CYAN); 
  ...
  spr.fillRect(XbatPos + 20, YbatPos + 2, 2, 5, TFT_CYAN); 
  spr.pushSprite(0, 0);
...
// Main loop OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
...
void giris() {
  tft.setTextSize(2);
  tft.setCursor(10, 20);
  tft.setTextColor(0x3526, TFT_BLACK);
  tft.println("SI4732 ALL BAND DSP RADYO");
  delay(1000);
  tft.setTextColor(0x3526, TFT_BLACK);
  tft.setCursor(38, 50);
  tft.print("ORJ. YAZILIM:");
  ...
  tft.setTextColor(0x3526, TFT_BLACK);
  tft.print("PiL GERiLiMi: ");
   tft.setTextColor(TFT_RED, TFT_BLACK);
  tft.print(battery.getBatteryVolts());
  tft.print(" V.");
  delay(3000);
  tft.fillScreen(TFT_BLACK);
...
// Arka ışığı kapatma kısmı
void bl_off(){
  ledcAttachPin(PIN_LCD_BL, 1);
 
}
// Arka ışığı açma kısmı
void bl_on() {
  ledcAttachPin(PIN_LCD_BL, 0);
}
