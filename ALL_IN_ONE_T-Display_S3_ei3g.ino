/*
  ...
  Yazılımda yapılan değişiklikler:
...
char sAgc[15];
const char *menu[] = {"Ses", "Adim", "Kip", "BFO", "BW", "AGC/Att", "SoftMute", "Yuk. Ara", "As. Ara", "Band", "Sessiz"};
...
Band band[] = {
    ...
    {"LW ", MW_BAND_TYPE, 150, 531, 356, 0, 5},
  ...
  if (currentMode != FM)
  {
    if (v == 1)  { // clockwise
      if (currentMode == AM)
      {
        ...
        spr.fillSmoothRoundRect(80,40,160,40,4,TFT_CYAN); //
...
void DrawBatteryLevel(int batteryLevel) {
  ...
  if ( batteryLevel == 2 ) {
    chargeLevel=9;
    batteryLevelColor=TFT_ORANGE; // (orj. YELLOW)
  }
  if ( batteryLevel == 3 ) {
    chargeLevel=13;
